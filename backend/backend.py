...
project = rf.workspace("trieu-l6awr").project("phanloaioc")
version = project.version(1)
dataset = version.download("yolov8")

# Đường dẫn tới thư mục chứa ảnh
dataset_dir = "./PhanLoaiOc/train/images"
labels_dir = "./PhanLoaiOc/train/labels"

# Tạo các thư mục train, val, test nếu chưa có
for folder in ['train', 'valid', 'test']:
    os.makedirs(f'./PhanLoaiOc/{folder}/images', exist_ok=True)
    os.makedirs(f'./PhanLoaiOc/{folder}/labels', exist_ok=True)
...
for i, img in enumerate(all_images):
    ...
    shutil.move(os.path.join(dataset_dir, img), os.path.join(f'./PhanLoaiOc/{folder}/images', img))
    shutil.move(os.path.join(labels_dir, img.replace(".jpg", ".txt")), os.path.join(f'./PhanLoaiOc/{folder}/labels', img.replace(".jpg", ".txt")))
...
# Huấn luyện mô hình
# model.train(data="./PhanLoaiOc/data.yaml", epochs=100)  # Thay "path/to/data.yaml" bằng đường dẫn đến file YAML của bạn

# Sử dụng mô hình để nhận diện vật thể trong hình ảnh
# test_image_path ="./images/OC(8).bmp"
...
# ===================================== Start Grab =====================================
# Đặt ngưỡng độ tự tin (confidence threshold)
CONFIDENCE_THRESHOLD = 0.8  # Bạn có thể điều chỉnh giá trị này tùy ý

# Khai báo bên ngoài route
seen_ids = set()

@app.route('/api/start_grab', methods=['POST'])
def start_grab():
    global camera, seen_ids
    if camera is None:
        return jsonify({"message": "Camera is not connected"}), 400
    ...
    try:
        # Kích hoạt camera và chụp ảnh
        grab_result = camera.GrabOne(50)
        if grab_result.GrabSucceeded():
            image = grab_result.Array
            grab_result.Release()

            # Chuyển ảnh sang file tạm
            _, buffer = cv2.imencode('.jpg', image)
            temp_image_path = 'temp_image.jpg'
            with open(temp_image_path, 'wb') as temp_file:
                temp_file.write(buffer)

            # Dùng YOLOv8 + ByteTrack
            results = model.track(temp_image_path, persist=True, tracker="bytetrack.yaml")
            ...
            for box in results[0].boxes:
                if hasattr(box, 'id') and box.id is not None:
                    ...
                    # Chỉ xử lý nếu object_id chưa được đếm và độ tự tin lớn hơn ngưỡng
                    if object_id not in seen_ids and confidence > CONFIDENCE_THRESHOLD:
                        ...
                        cv2.putText(
                            image,
                            f"{label} {confidence:.2f} (ID: {object_id})",
                            (x1, y1 - 10),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.5,
                            (0, 255, 0),
                        ...
                        # Thêm vào kết quả
                        detections.append({"label": label, "confidence": round(confidence, 2), "id": object_id})
        ...
        return jsonify({
            "message": f"Error: {e}",
            "cycle_time": f"{end_time - start_time:.2f}"
        }), 500
