...
function getCropCoords(obj, maxW, maxH) {
  ...
  let width = Math.min(Math.round(obj.getScaledWidth() / imageScale), maxW);
  let height = Math.min(Math.round(obj.getScaledHeight() / imageScale), maxH);
  let x = Math.max(
    0,
    Math.min(Math.round(centerX / imageScale - width / 2), maxW - 1)
  );
  let y = Math.max(
    0,
    Math.min(Math.round(centerY / imageScale - height / 2), maxH - 1)
  );
  ...
  let cropData = {
    ...
    shape: cropRegions.map((obj, index) => {
      ...
      return {
        ...
        angle: obj.angle || 0,
        cropX: Math.round(
          centerX / imageScale - obj.getScaledWidth() / (2 * imageScale)
        ),
        cropY: Math.round(
          centerY / imageScale - obj.getScaledHeight() / (2 * imageScale)
        ),
        cropWidth: Math.round(obj.getScaledWidth() / imageScale),
        cropHeight: Math.round(obj.getScaledHeight() / imageScale),
        result: false,
        score: 60,
      };
    }),
...
//   jsonData.shape.forEach(({ result ,shapeMode, cropX, cropY, cropWidth, cropHeight, angle = 0 }) => {
//     // Điều chỉnh kích thước hình dạng dựa trên imageScale
...
//     const commonProps = {
//       left: cropX + cropWidth / 2,
//       top: cropY + cropHeight / 2,
//       angle,
//       originX: "center",
//       originY: "center",
//       fill: "transparent",
//       stroke: result ? "greenyellow" : "red", // Màu viền của tất cả hình
//       strokeWidth: 2,
//       selectable: false, // Không cho phép chỉnh sửa
//     };

//     // Tạo hình dựa trên shapeMode
//     let shape;
//     if (shapeMode === "rect") {
//       shape = new fabric.Rect({ ...commonProps, width: cropWidth, height: cropHeight });
//     } else if (shapeMode === "circle") {
//       shape = new fabric.Circle({ ...commonProps, radius: cropWidth / 2 });
//     } else if (shapeMode === "triangle") {
//       shape = new fabric.Triangle({ ...commonProps, width: cropWidth, height: cropHeight });
//     }

//     canvas.add(shape); // Thêm hình vào canvas
//   });
// }
// ================== Vẽ lại các khung cắt từ file JSON ===================
function loadCropDataFromJSON(jsonData) {
  ...
  jsonData.shape.forEach(
    ({
      ...
      // Điều chỉnh kích thước hình dạng dựa trên imageScale
      const scaledCropWidth = cropWidth * imageScale;
      const scaledCropHeight = cropHeight * imageScale;
      const scaledCropX = cropX * imageScale;
      const scaledCropY = cropY * imageScale;

      // Xác định tâm của hình
      const centerX = scaledCropX + scaledCropWidth / 2;
      const centerY = scaledCropY + scaledCropHeight / 2;

      // Cấu hình chung cho mọi hình
      const commonProps = {
        left: centerX, // Đặt chính xác tại điểm cropX
        top: centerY, // Đặt chính xác tại điểm cropY
        angle, // Áp dụng góc xoay cho hình
        originX: "center",
        originY: "center",
        fill: "transparent",
        stroke: result ? `${colorOk}` : `${colorNG}`, // Màu viền thay đổi theo giá trị result
        strokeWidth: 2,
        selectable: false, // Không cho phép chỉnh sửa
      };
      ...
      console.log("scaledCropX:", scaledCropX);
      console.log("scaledCropWidth:", scaledCropWidth);

      // **Cách tính mới: scoreText luôn nằm sát cạnh phải trên của hình**
      const scoreTextX = scaledCropX + scaledCropWidth - 5; // Gần mép phải
      const scoreTextY = scaledCropY - 18; // Gần mép trên

      // Chuyển đổi góc
      const angleRad = (angle * Math.PI) / 180;
      // Tính lại vị trí sau khi xoay
      const rotatedX =
        centerX +
      ...
      // Tạo và thêm text hiển thị thông số score
      ...
      // Áp dụng xoay cho scoreText nếu cần
      scoreText.set({ angle });
...
//   .addEventListener("change", function (event) {
//     const file = event.target.files[0]; // Lấy file đầu tiên từ danh sách file người dùng chọn
...
//       reader.onload = function (e) {
//         try {
//           const data = JSON.parse(e.target.result); // Chuyển đổi dữ liệu từ JSON thành đối tượng JavaScript
...
//     } else {
//       console.log("Vui lòng chọn một file JSON.");
//     }
//   });
